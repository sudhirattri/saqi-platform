...
import AQIViewer from "./pages/AqiViewer";
...
function App() {
  ...
  useEffect(() => {
    if (voices) {
      if (voices.length > 0) {
        ...
        add_line_to_queue(
          getTranslated(userLanguage, "SplashMessage"),
          userLanguage
        );
        // runSpeech()
      }
  ...
  async function runSpeech() {
    ...
    let hin_voice = voices.find(
      (o) => o.voiceURI === "Microsoft Kalpana - Hindi (India)"
    );
    let eng_voice = voices.find(
      (o) => o.voiceURI === "Microsoft Ravi - English (India)"
    );
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Container
        component="main"
        maxWidth="sm"
        sx={{ mb: 0, backgroundColor: "rgba(255,255,255,0.8)" }}
        style={{ height: "100vh" }}
      >
        <BrowserRouter>
          <Routes>
